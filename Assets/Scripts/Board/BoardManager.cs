...
public class BoardManager : MonoBehaviour
{
    ...
    public Player currentTurn = Player.X; 
    ...
    private void Start()
    {
        // 🌟 FIX: Pull the selected match type dynamically from GameManager if it exists
        if (GameManager.Instance != null)
        {
            playAgainstAI = GameManager.Instance.playAgainstAI;
            Debug.Log($"[BOARD] Mode Sync Complete! playAgainstAI = {playAgainstAI}");
        }
    ...
    public void OnCellClicked(Cell clickedCell, bool force = false)
    {
        ...
        // 🌟 FIX: Only block Player O's clicks if we are ACTUALLY playing against an active AI!
        if (currentTurn == Player.O && playAgainstAI && !force) return;

        if (isAIThinking && !force) return; 
    ...
    /// <summary>
    /// Master clean teardown function linked directly to your HUD Reset Button
    /// </summary>
    public void ResetUltimateMatch()
    {
        Debug.Log("🧼 Resetting match engine data and clearing visual pieces...");

        // 1. Wipe out all physical token game objects on screen cleanly
        ...
        // 2. Clear array memory maps back to factory clean values
        globalBoardData = new CellState[9, 9];
        
        for (int i = 0; i < 9; i++)
        {
            miniBoardStates[i] = BoardState.Active;
        }
        ...
        // 3. Reset background bounding cell panels back to full visibility
        ...
        // 4. Force state alignment hooks to sync visual updates
        RefreshBoardHighlights();
        if (UIManager.Instance != null)
        {
    ...
    private void CheckMiniBoardWin(int boardIndex, Player player)
    {
        ...
        foreach (int[] pattern in winPatterns)
        {
            ...
            {
                // 🌟 FIX: Change WonX / WonO to WonByX / WonByO
        ...
        // 🌟 FIX: Change BoardState.Draw to BoardState.Tied to match your enum
